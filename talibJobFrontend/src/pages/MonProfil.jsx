...
export default function MonProfil() {
  ...
  const handleSave = async () => {
    ...
    setMessage(null);
    try {
      await api.put("/api/etudiant/profil", form);
      const r = await api.get("/api/etudiant/profil");
      setProfil(r.data);
      setForm({
        nom:         r.data.nom         || "",
        prenom:      r.data.prenom      || "",
  ...
  const handlePhotoChange = async (e) => {
    ...
    try {
      const r = await api.post("/api/etudiant/upload-photo", formData, {
  ...
  const handleCvChange = async (e) => {
    ...
    try {
      await api.post("/api/etudiant/upload-cv", formData, { headers: { "Content-Type": "multipart/form-data" } });
  ...
  const fullName = `${profil?.prenom || ""} ${profil?.nom || ""}`.trim() || "Étudiant";
  ...
  return (
    <div>
      ...
      <div className="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-3">
        <div>
          <h2 className="fw-bold mb-1" style={{ fontSize: 22 }}>Mon Profil</h2>
          <p className="text-muted mb-0" style={{ fontSize: 13 }}>Gérez vos informations personnelles</p>
        </div>
        {!editMode ? (
          <button className="btn btn-primary btn-sm d-flex align-items-center gap-2"
            style={{ borderRadius: 8 }} onClick={() => setEditMode(true)}>
          ...
          <div className="card p-4 text-center" style={{ borderRadius: 14, border: "1px solid #e8eaf0" }}>
            ...
            <div className="mb-3">
              <div className="d-flex justify-content-between mb-1">
                ...
                <small className="fw-bold" style={{ color: "#3b82f6" }}>{profil?.progression || 0}%</small>
              </div>
              <div className="progress" style={{ height: 6, borderRadius: 6 }}>
                <div className="progress-bar" style={{ width: `${profil?.progression || 0}%`, background: "#3b82f6" }} />
              </div>
            </div>

            {/* Upload photo */}
            <label className="btn btn-outline-secondary btn-sm w-100 mb-2" style={{ borderRadius: 8, cursor: "pointer" }}>
            ...
            <div className="d-flex flex-column gap-2 mt-2">
              {profil?.cv && (
                <a href={`http://localhost:8000/storage/${profil.cv}`}
                  className="btn btn-outline-primary btn-sm" style={{ borderRadius: 8 }}
                  target="_blank" rel="noreferrer">
                  <i className="bi bi-download me-1" />Télécharger CV
                </a>
              )}
              <label className="btn btn-outline-secondary btn-sm" style={{ borderRadius: 8, cursor: "pointer" }}>
                ...
                <input type="file" accept=".pdf" className="d-none" onChange={handleCvChange} />
              </label>
            </div>
          </div>
        </div>
                ...
                <div className="row g-3">
                  {[
                    ...
                    <div key={f.label} className="col-sm-6">
                      ...
                      <span className="fw-semibold" style={{ fontSize: 14 }}>{f.value || "—"}</span>
                    </div>
                  ))}
                </div>
                  ...
                  {(profil?.competences || []).length > 0
                    ? profil.competences.map((c, i) => (
                        <span key={i} className="badge"
                          style={{ background: "#eff6ff", color: "#1d4ed8", fontSize: 12, fontWeight: 600, padding: "5px 12px", borderRadius: 8 }}>
                          {c}
                        </span>
                      ))
                    : <span className="text-muted small">Aucune compétence renseignée</span>
                  }
                </div>
              </>
            ) : (
              ...
              <>
                ...
                <div className="mb-3">
                  <label className="form-label fw-semibold small">
                    Compétences <small className="text-muted fw-normal">(séparer par des virgules)</small>
                  </label>
                  <input type="text" className="form-control" style={{ borderRadius: 8 }}
                    placeholder="React, PHP, Laravel..."
                    value={form.competences} onChange={e => setForm({ ...form, competences: e.target.value })} />
                </div>
              </>
            )}
          </div>
